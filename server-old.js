...
function startGameTimer(game, io) {
  console.log(`⏰ Démarrage du timer pour la partie ${game.id}`);
  
  const timer = setInterval(() => {
    // Vérifier si la partie est toujours active
    if (game.status !== 'active') {
      console.log(`⏰ Timer arrêté - partie ${game.id} terminée`);
      clearInterval(timer);
      return;
    }
    
    // Vérifier si c'est une partie amicale (pas de timer)
    if (game.isFriendlyGame) {
      console.log(`⏰ Timer arrêté - partie amicale ${game.id}`);
      clearInterval(timer);
      return;
    }
    
    // Décrémenter 1 seconde du joueur actuel
    const timeDecrement = 1000; // 1 seconde en millisecondes
    
    if (game.currentPlayer === 'white') {
      game.timeLeft.white = Math.max(0, game.timeLeft.white - timeDecrement);
    } else if (game.currentPlayer === 'black') {
      game.timeLeft.black = Math.max(0, game.timeLeft.black - timeDecrement);
    }
    
    // Vérifier les timeouts
    if (game.timeLeft.white <= 0) {
      console.log(`⏰ Timeout - les blancs ont perdu`);
    ...
    if (game.timeLeft.black <= 0) {
      console.log(`⏰ Timeout - les noirs ont perdu`);
      game.status = 'finished';
      game.winner = 'white';
      game.winReason = 'timeout';
  ...
  // Stocker la référence du timer pour pouvoir l'arrêter
  game.timer = timer;
